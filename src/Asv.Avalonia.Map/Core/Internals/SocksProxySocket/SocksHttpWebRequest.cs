...
namespace Asv.Avalonia.Map
{
    ...
    internal class SocksHttpWebRequest : WebRequest
    {
        ...
        static readonly StringCollection ValidHttpVerbs = new StringCollection
        {
            "GET",
            "HEAD",
            "POST",
        ...
        private SocksHttpWebResponse InternalGetResponse()
        {
            ...
            using (
                var socksConnection = new ProxySocket(
            ...
            )
            {
                ...
                while ((bytesReceived = socksConnection.Receive(buffer)) > 0)
                {
                    if (!headerDone)
                    {
                        string headPart = Encoding.UTF8.GetString(
                            buffer,
                            0,
                            bytesReceived > 1024 ? 1024 : bytesReceived
                        );
                        ...
                        if (indexOfFirstBlankLine > 0)
                        {
                            ...
                            if (headerPartLength < bytesReceived)
                            {
                                ...
                                data.Write(
                                    buffer,
                                    headerPartLength,
                                    bytesReceived - headerPartLength
                                );
                            }
                        }
                        else
